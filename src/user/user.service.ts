...
import { BAD_REQUEST, CREATED, OK, UNAUTHORIZED } from 'src/constants/code.constant';
...
export class UserService {
  ...
  async delete(id: string): Promise<Response> {
    const response = this.userModel.findByIdAndDelete(id).then((response) => {
      ...
      return returnSuccessMessage([Message.LOGIN_SUCCESS_MESSAGE], OK, new ResponseUserDTO(response.name, response.email, response.role));
    }).catch((error) => {
